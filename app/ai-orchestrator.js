...
(function (root, factory) {
  ...
  async function orchestrate(question, currentBoard, deps, opt) {
    ...
    /* 2026-09-11 实测：专家轮次耗尽时交上来的是「I have the core data. Let me pull the monthly trend…」这种 200 字的过程话，
       比 150 字长就漏过了半途检测，综合层拿着过程话写出「数据均未取到」。以过程句开头的一律算半途（调用方已限定 claims 为空）。 */
    const PROCESS_START_RE = /^(I have|I now have|I'?ll|I will|Let me|I need|Now let me|Both calls|The catalog says|让我|我来|我先|现在我|我需要|接下来)/i;
    /* 「过程话开头」还要看后面有没有正文：「I have everything I need. Here's the summary. ## 结论 …」是带一句过场的正经回答，
       不能当半途重跑（会把好答案扔掉）。只有整段没有结论/表格/像样的数字时才算半途。 */
    const hasBody = (x) => /(结论|建议|同比|\|[^|]+\||\d[\d,]{3,})/.test(x);
    const HALFWAY_RE = { test: (t) => { const x = String(t || '').trim(); return (x.length < 150 && HALFWAY_WORDS.test(x)) || (PROCESS_START_RE.test(x) && !hasBody(x)); } };
  ...
  return {
    ...
    scanEntities, understandInContext, planWithLLM, agentDuty, provenanceRetry, analysisPlaybook, FOLLOWUP_RE,
  };
});
